...
class App extends React.Component {
    constructor(props) {
        ...
        for (var i =0; i < 10; i++)
        {
          this.yearRef[i] = React.createRef();
        }
        this.itemArray = [];
        this.galleryArray = [];
    ...
    componentDidMount() {
        this.imageRef.current.addEventListener("mousedown", this.sliderMouseDown);
        this.imageRef.current.addEventListener("mouseleave", this.sliderMouseLeave);
        this.imageRef.current.addEventListener("mouseup", this.sliderMouseUp);
        this.imageRef.current.addEventListener("mousemove", this.sliderMouseMove);
        this.yearRef[0].current.addEventListener("mousedown", this.year1MouseDown);
        this.yearRef[1].current.addEventListener("mousedown", this.year2MouseDown);
        this.yearRef[2].current.addEventListener("mousedown", this.year3MouseDown);
        this.yearRef[3].current.addEventListener("mousedown", this.year4MouseDown);
        this.yearRef[4].current.addEventListener("mousedown", this.year5MouseDown);
        this.yearRef[5].current.addEventListener("mousedown", this.year6MouseDown);
        this.playRef.current.addEventListener("mousedown", this.playstart);
      ...
      else
      {
        ...
        for(var i =0; i < 6; i++)
        {
          if (this.yearRef[i].current.classList.contains('on')) 
            this.yearRef[i].current.classList.remove('on');
        }
        this.yearRef[intindex].current.classList.add('on');
        for (var i = 0; i < 6; i++) {
            if (i == intindex) 
            {
              this.itemArray[i].current.style.display = 'block';
              this.galleryArray[i].current.style.display = 'block';
            } 
            else 
            {
              this.itemArray[i].current.style.display = 'none';
                this.galleryArray[i].current.style.display = 'none';
            }
        }
        for (var i = 0; i < 16; i++) 
        {
    ...
    actuactorUp = (fromYear, toYear ,playflag) => {
      ...
      else
      {
        ...
        {
          if(year_list[i]>= fromYear && fromIndex == -1)
          {
            fromIndex = i;
          }
          if(year_list[i] > toYear)
          {
            toIndex = i-1;
            break;
          }
          ...
          if(i == year_list.length-1 && toIndex == -1)
          {
            toIndex = i;
          }
        }
    ...
    playstart = () => {
      ...
      if(!this.playRef.current.classList.contains('on'))
        this.playRef.current.classList.add('on');
      for (var i = 0; i < 16; i++) {
        clearTimeout(this.playtimer[i]);
      }
      //#endregion playmode 동작
      this.actuactorUp(1990,2020,true);
    }
    fullScreen = () => {
      if(!this.fullTrigRef.current.classList.contains('on'))
      {
    ...
    closeModal = () => {
        ...
        this.mainBody.current.classList.remove("not-scroll");
    ...
    render() 
    {
      this.socket.on('connect', () => {
        ...
        this.socket.emit('message', "I'm client1.");
      });
              ...
              <div className="grid-container" >
                <main className="grid-item main">
                  <div className="items" ref={this.imageRef}>
                    <div className="s_item item1 on" ref={this.yearRef[0]} >1990~1995</div>
                    <div className="s_item item2" ref={this.yearRef[1]} >1996~2000</div>
                    <div className="s_item item3" ref={this.yearRef[2]} >2001~2005</div>
